...
/**
 * Purpose of this class is to server as the main server that listens for new connections from clients. 
 * 
 * 
 * @author James Harris
...
public class ConnectionListener {
	...
	public static void main(String args[]){
		...
		try {
			...
			File theLib = new File(System.getProperty("user.dir") + "/Library/" + "serverLib.xml");
			if (!theLib.exists()) {
				System.out.println("creating Library: "
						+ System.getProperty("user.dir") + "/Library/" + "serverLib.xml");
			...
			lib = lib.restore(System.getProperty("user.dir") + "/Library/" + "serverLib.xml");		
			ServerSocket serv = new ServerSocket(portNo);
			System.out.println(portNo);
			Vector<ClientThread>clients = new Vector<ClientThread>();
			while (true) {
				System.out.println("Threaded server waiting for connects on port "
						+portNo);
				clients.add(new  ClientThread(serv.accept(),id++,lib,clients, portNo));
